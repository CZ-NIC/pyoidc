...
def assertion_jwt(cli, keys, audience, algorithm, lifetime=600):
    ...
    logger.debug("AuthnToken: %s", at.to_dict())
...
class ClientSecretBasic(ClientAuthnMethod):
    ...
    def construct(self, cis, request_args=None, http_args=None, **kwargs):
        ...
        if (("client_id" not in cis.c_param.keys()) or cis.c_param["client_id"][VREQUIRED]) is False:
            try:
                del cis["client_id"]
            except KeyError:
                pass
...
class JWSAuthnMethod(ClientAuthnMethod):
    ...
    def construct(self, cis, request_args=None, http_args=None, **kwargs):
        ...
        if kwargs["authn_endpoint"] in ["token", "refresh"]:
            try:
                algorithm = self.cli.registration_info["token_endpoint_auth_signing_alg"]
        ...
        try:
            if "kid" in kwargs:
                ...
                try:
                    signing_key = [self.get_key_by_kid(self.cli.kid["sig"][ktype], algorithm)]
                except KeyError:
        ...
        if "client_assertion" in kwargs:
            ...
            cis["client_assertion"] = assertion_jwt(self.cli, signing_key, audience, algorithm, **_args)
...
def verify_client(inst, areq, authn, type_method=TYPE_METHOD):
    ...
    if authn:  # HTTP Basic auth (client_secret_basic)
        ...
        for typ, method in type_method:
            ...
            logger.error("UnknownAssertionType: %s", areq["client_assertion_type"])
    ...
    if isinstance(areq, AccessTokenRequest):
        ...
        if _method != auth_method:
            logger.error("Wrong authentication method used: %s != %s", auth_method, _method)
