...
def opbyuid(environ, start_response):
    resp = Response(mako_template="opbyuid.mako", template_lookup=LOOKUP, headers=[])
    argv = {}
    return resp(environ, start_response, **argv)


def post_logout(environ, start_response):
    resp = Response(mako_template="post_logout.mako", template_lookup=LOOKUP, headers=[])
    argv = {}
    return resp(environ, start_response, **argv)


def choose_acr_value(environ, start_response, session):
    resp = Response(mako_template="acrvalue.mako", template_lookup=LOOKUP, headers=[])
    argv = {"acrvalues": session["acr_values"]}
    return resp(environ, start_response, **argv)
...
def application(environ, start_response):
    ...
    if path == "logout":
        try:
            ...
            try:
                logoutUrl += "&" + urlencode({"id_token_hint": id_token_as_signed_jwt(session["client"], "HS256")})
...
if __name__ == "__main__":
    ...
    RP = OpenIDConnect(registration_info=conf.ME, ca_bundle=conf.CA_BUNDLE)

    SRV = wsgiserver.CherryPyWSGIServer(
        ("0.0.0.0", conf.PORT),  # nosec
