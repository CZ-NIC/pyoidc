...
class OIDCExampleRP(object):
    ...
    def register_with_dynamic_provider(self, session, uid):
        ...
        session["client"].register(provider_info["registration_endpoint"], **self.client_metadata)
    ...
    def make_authentication_request(self, session):
        ...
        auth_req = session["client"].construct_AuthorizationRequest(request_args=request_args)
    ...
    def make_token_request(self, session, auth_code):
        ...
        token_response = session["client"].do_access_token_request(
            scope="openid", state=session["state"], request_args=args
        )

        return token_response

    def make_userinfo_request(self, session, access_token):
        userinfo_response = session["client"].do_user_info_request(access_token=access_token)
...
class RPServer(object):
    ...
    def repost_fragment(self, **kwargs):
        response = self.rp.parse_authentication_response(cherrypy.session, kwargs["url_fragment"])
        ...
        try:
            ...
            userinfo = self.rp.make_userinfo_request(cherrypy.session, access_token)
        except KeyError:
            pass
        return html_page.format(authz_code, access_token, response["id_token"], userinfo)
    ...
    def code_flow(self, **kwargs):
        ...
        auth_response = self.rp.parse_authentication_response(cherrypy.session, qs)
...
def main():
    ...
    registration_info["redirect_uris"] = [
        url.format(base=baseurl, port=args.port) for url in registration_info["redirect_uris"]
    ]

    rp_server = RPServer(registration_info, settings["behaviour"], settings["server"]["verify_ssl"])
